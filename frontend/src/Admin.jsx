...
import { Trophy, Swords, LayoutList, Users, LogOut, FileUp, Monitor, PlayCircle, Settings, PlusCircle, Gamepad2, UserPlus } from 'lucide-react';
...
export default function Admin() {
  ...
  const createPair = async () => { 
    if (!selectedC || !p1Name || !p2Name) return alert('Preecha os dois atletas');
    const fullName = `${p1Name} / ${p2Name}`;
    await supabase.from('pairs').insert([{ category_id: selectedC, name: fullName }]); 
    setP1Name(''); setP2Name('');
    ...
    if (action === '+pt') {
      if (teamIdx === 1) { 
        if (m.pair1_score === '40') { 
          ...
          m.pair1_games += 1; 
          if (m.pair1_games >= 6) { m.pair1_sets += 1; m.pair1_games = 0; m.pair2_games = 0; } 
        ...
        if (m.pair2_score === '40') { 
          m.pair1_score = '0'; m.pair2_score = '0'; 
          m.pair2_games += 1; 
          if (m.pair2_games >= 6) { m.pair2_sets += 1; m.pair1_games = 0; m.pair2_games = 0; } 
  ...
  if (!session) return (
    <div className="admin-login-screen" style={{height:'100vh', display:'flex', justifyContent:'center', alignItems:'center', background:'#000'}}>
    ...
    <div className="admin-layout">
      <aside className="admin-sidebar glass-panel">
        ...
        <nav className="admin-nav">
          <div className={`nav-link ${activeTab === 'scoreboard' ? 'active' : ''}`} onClick={() => setActiveTab('scoreboard')}> <Swords size={18} /> Placar Ativo</div>
          <div style={{height: 20}}></div>
          <div className={`nav-link ${activeTab === 'tournaments' ? 'active' : ''}`} onClick={() => setActiveTab('tournaments')}> <LayoutList size={18} /> 1. Torneio & Categorias</div>
          <div className={`nav-link ${activeTab === 'pairs' ? 'active' : ''}`} onClick={() => setActiveTab('pairs')}> <UserPlus size={18} /> 2. Cadastro de Duplas</div>
          <div className={`nav-link ${activeTab === 'matches' ? 'active' : ''}`} onClick={() => setActiveTab('matches')}> <Gamepad2 size={18} /> 3. Organizar Partidas</div>
          <div className={`nav-link ${activeTab === 'import' ? 'active' : ''}`} onClick={() => setActiveTab('import')}> <FileUp size={18} /> LetzPlay</div>
          <div style={{flex: 1}}></div>
          <a href="/tv" target="_blank" className="nav-link" style={{color:'var(--accent-primary)', textDecoration:'none'}}><Monitor size={18}/> Ver na TV</a>
      ...
      <main className="admin-main">
        {activeTab === 'scoreboard' && (
          <div className="admin-view-container">
            <h2 className="view-title">Placar em Tempo Real</h2>
            <div className="matches-list">
              {matches.map(m => (
                <div key={m.id} className="glass-panel match-ctrl-card">
                  <div className="card-header">
                    <span className="cat-badge">{m.category_name}</span>
                    <span style={{color: m.status === 'in_progress' ? 'var(--accent-primary)' : '#666', fontWeight:'bold'}}>{m.status.toUpperCase()}</span>
                  </div>
        ...
        {activeTab === 'pairs' && (
          <div className="admin-view-container" style={{maxWidth:700}}>
            <h2 className="view-title">Registro de Duplas</h2>
            ...
            <div className="glass-panel" style={{padding:30, borderRadius:20}}>
               ...
               {selectedC && <div>
                  ...
                  <div style={{display:'flex', gap:10, marginBottom:20, alignItems:'center'}}>
                     ...
                     <div style={{fontWeight:'bold', color:'var(--accent-primary)'}}>X</div>
